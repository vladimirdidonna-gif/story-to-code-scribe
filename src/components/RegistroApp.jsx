      ...
      <Modal open={!!mpOpen} onClose={()=>setMpOpen(false)} width={900} headerColor={TEAL}
        ...
        <div style={{padding:"16px 24px",borderBottom:"1px solid #eee",display:"flex",flexDirection:"column",gap:10,fontFamily:FF}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr 1fr",gap:10}}>
            ...
            <div><div style={{fontSize:12,color:TEAL,fontWeight:600,marginBottom:3}}>Peso</div><div style={{display:"flex",alignItems:"center",gap:4}}><input type="number" min="0" max="100" value={mpForm.peso} onChange={e=>{const raw=e.target.value;setMpForm({...mpForm,peso:raw===""?"":Math.min(100,Math.max(0,Number(raw)))})}} onBlur={()=>{if(mpForm.peso==="")setMpForm(f=>({...f,peso:100}))}} style={{flex:1,border:"1px solid #ccc",borderRadius:4,padding:"7px 6px",fontSize:13,fontFamily:FF}}/><span style={{fontSize:11,color:"#888"}}>%</span></div></div>
